import discord, random, time, praw, datetime, asyncio
...
async def on_member_remove(member):
    ...
    embed = discord.Embed(title = "Goodbye!", description = f"Goodbye {member.mention}. Hope you enjoyed your stay with us.", colour = white)   
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        embed = discord.Embed(title = "**Insufficient power!**", description = f"You dont have the power to do that '-'", colour = discord.Colour.red())  
        await ctx.send(embed = embed)
    elif isinstance(error, commands.MissingRequiredArgument):
        embed = discord.Embed(title = "**Insufficient input!**", description = f"Please enter all the required arguments.", colour = discord.Colour.red()) 
        await ctx.send(embed = embed)
    else:
        raise error

@client.command(name = "help", description = "Shows this message")
async def help(ctx, cmd : str = None):
    if cmd != None:
        ...
        embed = discord.Embed(title = "Standard Commands", description = f"Type {prefix}help <command> for more help e.g. !help kick", colour = white)           

        for cmd in client.commands:
            embed.add_field(name = cmd.name.lower(), value = cmd.description, inline = True)
...
@client.command(name = "ping", description = "Used to test Birth Control's response time.", aliases = ["p"])
async def ping(ctx):
    embed = discord.Embed(title = "Ping", description = f":information_source:  **|**  Pong! - Time taken: **{str(round(client.latency * 1000))}ms**", colour = white)   
    await ctx.send(embed = embed)
...
@client.command(name = "kick", description = "Kicks a user from the server.", aliases = ["k"])
@commands.has_permissions(kick_members = True)
async def kick(ctx, member : discord.Member,*,reason = "No reason provided"):
    await member.send(f"You have been kicked from Cum Clicker\nreason: {reason}")
    await ctx.send(f"{member.mention} has been kicked from {ctx.guild.name}\nreason: {reason}")
    await member.kick(reason = reason)
    
@client.command(name = "ban", description = "Bans a user from the server.", aliases = ["b"])
@commands.has_permissions(ban_members = True)
async def ban(ctx, member : discord.Member,*,reason = "No reason provided"):
    embed = discord.Embed(title = "Ban", description = f"", colour = discord.Colour.red())   
    await member.send(f"You have been banned from Cum Clicker\nreason: {reason}")
...
@client.command(name = "meme", description = ".")
async def meme(ctx):
...
@client.command(name = "cat", description = "Gives you a random cat :3", aliases = ["neko"])
async def cat(ctx):
...
@client.command(name = "dog", description = "Gives you a random dog :3", aliases = ["inu"])
async def dog(ctx):
...
@client.command(name = "rps", description = ".", aliases = ["rockpaperscissors"])        
...
@client.command(name = "giveaway", description = ".", aliases = [])
@commands.has_permissions(administrator = True)
...
async def fortune(ctx):
    embed = discord.Embed(title = "**Error! MissingCommand**", description = f"That command does not exst '-'.", colour = discord.Colour.red())            
    await ctx.send(embed = embed)
